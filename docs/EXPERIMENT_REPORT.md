...
### 實驗記錄 ID: 20260206-AB-4-HASH-BITS

**日期**: 2026-02-06  
**類別**: Ablation Study (AB-4)  
**變更摘要**: 測試不同 Hash Bits 對效能的影響。

#### 1. 實驗配置

- **模型**: SigLIP2 + Fusion + Hash Layer
- **Hash Bits**: 32 / 64 (基準) / 128 / 256
- **Loss**: BCE + Cosine + Hash (完整 AGCH)

#### 2. 實驗結果

| Hash Bits | mAP | AUC | Epoch | 說明 |
| :---: | :---: | :---: | :---: | :--- |
| 32 | **0.7495** | - | 43 | ✅ 比 64-bit 更好 |
| 64 (基準) | 0.6787 | 0.9613 | - | AGCH 原始設定 |
| 128 | **0.7899** | - | - | ✅ 顯著回升 (+11.1%) |
| 256 | - | - | - | 待執行 |

#### 3. 初步分析

1. **32-bit (0.7495) > 64-bit (0.6787)**：較小的 Hash 空間可能有助於減少過度正則化。
2. **128-bit (0.7899) > 32-bit**：隨著 Hash 容量增加，資訊保留能力提升，效能顯著回升，逼近 No Hash (0.8323) 的水準。
3. **推論**：64-bit 可能是特定的局部極小值 (或者超參數不適配)。增加 Bits 數似乎是正確方向。建議繼續測試 256-bit。
